...
class MessengerService {
    async createRoom(req, res) {
        ...
        await ChatRoom.create({user1, user2})
        res.json({msg: 'success'})
    }
    async getRooms(req, res) {
        const user = req.user.userId

        let rooms1 = await ChatRoom.find({user1: user})
        let rooms2 = await ChatRoom.find({user2: user})
    ...
    async getRoom(req, res) {
        ...
        if(!room) {
            room = await ChatRoom.findOne({user1: user2, user2: user1})
        }
        res.json({room})
    }
    ...
    async checkRooms(req, res) {
        ...
        const room1 = await ChatRoom.findOne({user1, user2})
        const room2 = await ChatRoom.findOne({user1: user2, user2: user1})
